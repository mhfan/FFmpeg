...
#endif	/* comment by mhfan */
{
    ...
    i = s->nb_channels * BLOCK_MAX_SIZE * sizeof(fixed32);
    ...
    if (s->version == 2 && avctx->extradata_size >= 6) {
        flags1 = extradata[0] | (extradata[1] << 8) |
		(extradata[2] << 16) | (extradata[3] << 24);
        flags2 = extradata[4] | (extradata[5] << 8);
    }
	...
	void *data, int *data_size, AVPacket* avpkt)
{
	...
	if (wma_decode_superframe_frame(s, (uint8_t*)data + *data_size,
		buf, buf_size) < 0) return -1;
	else *data_size += size;
